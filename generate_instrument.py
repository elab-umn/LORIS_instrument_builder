...
def convert_instrument_template(instrument_information, **kwargs):
    ...
    json_keys_instrument_parameters = [
        "instrument_name_sql",
        "instrument_name",
        "metadata_fields",
        "validity_enabled",
    ...
    if "instrument_name" not in instrument_parameters.keys():
        instrument_parameters["instrument_name"] = instrument_parameters[
            "sql_table_name"
        ]
    else:
        instrument_parameters["instrument_name"] = clean_strings_for_php(
            instrument_parameters["instrument_name"]
        )
    ...
    if "pages" in instrument_information.keys():
        for pg in instrument_information["pages"].keys():
            ...
            if (
                pg in ["page0", "main"]
                or instrument_information["pages"][pg]["title"] == "Top Page"
            ):
                tmp_page_key = "page0"
                    ...
                    tmp_g["group_fields_keys"] = [
                        {
                            ...
                            "label": value.get(
                                "field_front_text_php", value["field_name_sql"]
                            ),
                            "type": [
                                x
                                for x in [
                                    k if (value["field_type_sql"] in v) else None
                            ...
                            "options": (
                                ...
                                + ")"
                                if value["field_type_sql"] == "enum"
                                else False
                            ),
                            # }
                            # "customs": None
                        }
                        for key, value in instrument_information["fields"].items()
                        if value["field_name_sql"]
                        in instrument_information["groups"][g]["group_fields"]
                    ]
    ...
    if "fields" in instrument_information.keys():
        ...
        for q in instrument_information["fields"].keys():
            ...
            for param in json_keys_field_parameters:
                ...
                if param in instrument_information["fields"][q].keys():
                    # ------------------------------- format enums ------------------------------- #
                    if (
                        param == "enum_values_sql"
                        and instrument_information["fields"][q]["field_type_sql"]
                        == "enum"
                    ):
                        # IF we do not have enum fields provided
                        if not bool(
                            instrument_information["fields"][q]["enum_values_sql"]
                        ):
                            fields_warnings[f"{q}"] = (
                                "Question listed as enum, but no enum_values_sql provided."
                            )
                        # or IF we have enum_values_sql and enum_values_php
                        elif bool(
                            instrument_information["fields"][q]["enum_values_sql"]
                        ) and bool(
                            instrument_information["fields"][q].get(
                                "enum_values_php", []
                            )
                        ):
                            ...
                            if len(
                                ...
                                fields_warnings[f"{q}"] = (
                                    "Length of enum_values_sql does not equal enum_values_php. Did not apply enum_values_php to template"
                                )
                                tmp_q["enum_array"] = (
                                    "array("
                                    + ",".join(
                                        [
                                            f'"{key1}"'
                                            for key1 in instrument_information[
                                                "fields"
                                            ][q]["enum_values_sql"]
                                        ]
                                    )
                                ...
                                tmp_q["enum_array"] = (
                                    ...
                                    + ",".join(
                                        [
                                            f'"{key1}" => "{key2}"'
                                            for key1, key2 in zip(
                                                instrument_information["fields"][q][
                                                    "enum_values_sql"
                                                ],
                                                instrument_information["fields"][q][
                                                    "enum_values_php"
                        ...
                        # otherwise we have enum_values_sql, and just save those values
                            ...
                            tmp_q["enum_array"] = (
                                ...
                                + ",".join(
                                    [
                                        f'"{key1}"'
                                        for key1 in instrument_information["fields"][q][
                                            "enum_values_sql"
                                        ]
                                    ]
                                )
                    ...
                    #     tmp_q["enum_array"] = "array(" + ",".join([f'"{key1}"' for key1 in instrument_information["fields"][q]["enum_values_sql"]]) + ")"
                    # ----------------------------- format XIN Rules ----------------------------- #
                    if param == "rule_php" and bool(
                        instrument_information["fields"][q]["rule_php"]
                    ):
                        # TODO: test this works, and add error IO handling
                        tmp_q["XINRegisterRule"] = {
                            "field_name": instrument_information["fields"][q][
                                "field_name_sql"
                            ],
                            "rule": (
                                instrument_information["fields"][q]["rule_php"]["rule"]
                                if (
                                    "rule"
                                    in instrument_information["fields"][q][
                                        "rule_php"
                                    ].keys()
                                )
                                else False
                            ...
                            "group": (
                                instrument_information["fields"][q]["rule_php"]["group"]
                                if (
                                    "group"
                                    in instrument_information["fields"][q][
                                        "rule_php"
        ...
        raise OSError(
            "Instrument Template file doesn't have any fields, or they are incorrectly labelled. Should be 'fields': \{\}"
        )
...
def generate_instrument_sql(instrument_data):
    ...
    with open(
        "templates/LORIS_INSERT_test_names_template.sql.jinja2"
    ) as filein:  # noqa
        test_names_template = Template(
            filein.read(), trim_blocks=True, lstrip_blocks=True
        )
        # TODO: add support for instrument subgroup input

    with open(
        "outputs/sql/test_names_INSERT_"
        + instrument_data["instrument"]["sql_table_name"]
        + ".sql",
        "w",
    ) as output:
    ...
    # --------------------- INSERT into test_subgroups table --------------------- #
    with open(
        "templates/LORIS_INSERT_instrument_subtests_template.sql.jinja2"
    ) as filein:  # noqa
        instrument_subtest_template = Template(
            filein.read(), trim_blocks=True, lstrip_blocks=True
        )

    with open(
        "outputs/sql/instrument_subtests_INSERT_"
        + instrument_data["instrument"]["sql_table_name"]
        + ".sql",
        "w",
    ) as output:
...
def generate_instrument_from_template(instrument_json, output_dir):
    ...
    with open(
        os.path.join(
            output_dir,
            "json",
            f"instrument_{inst['instrument']['sql_table_name']}.json",
        ),
        "w+",
    ) as output:
        # output.write(json.dumps(instrument_json))
        output.seek(0)
        json.dump(instrument_json, output, indent=4)
    ...
    with open(
        "templates/LORIS_CREATE_instrument_table_template.sql.jinja2"
    ) as filein:  # noqa
    ...
    with open(
        os.path.join(
            output_dir,
            "sql",
            f"instrument_CREATE_{inst['instrument']['sql_table_name']}.sql",
        ),
        "w+",
    ) as output:
    ...
    with open(
        os.path.join(
            output_dir,
            "sql",
            "test_names_INSERT_" + inst["instrument"]["sql_table_name"] + ".sql",
        ),
        "w+",
    ) as output:
    ...
    with open(
        os.path.join(
            output_dir,
            "sql",
            "instrument_subtests_INSERT_"
            + inst["instrument"]["sql_table_name"]
            + ".sql",
        ),
        "w+",
    ) as output:
    ...
    with open(
        os.path.join(
            output_dir,
            "php",
            "NDB_BVL_Instrument_" + inst["instrument"]["sql_table_name"] + ".class.inc",
        ),
        "w+",
    ) as output:
