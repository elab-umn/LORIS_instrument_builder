    ...
    if path:
        ...
        if source == "qualtrics":
            if args.project != None:
                ...
                token = valid_config_parameter(
                    "config/qualtrics_config.ini", args.project, "api-token"
                )
                datacenter = valid_config_parameter(
                    "config/qualtrics_config.ini", args.project, "data-center"
                )
                surveylabel = (
                    args.survey
                    if (args.survey != None)
                    else input(
                        "Which survey do you want to create? (should match label in 'config/qualtrics_survey_import_config.ini')"
                    )
                )
                survey = valid_config_parameter(
                    "config/qualtrics_survey_import_config.ini",
                    args.project,
                    surveylabel,
                )
            else:
                ...
                token = (
                    ...
                    else input("Enter Qualtrics API token: ")
                )
                datacenter = (
                    args.datacenter
                    if (args.datacenter != None)
                    else input("Enter Qualtrics Datacenter: ")
                )
                survey = (
                    args.survey
                    if (args.survey != None)
                    else input("Enter Qualtrics Survey ID: ")
                )
            ...
            if args.survey != None:
                ...
                instrument["instrument_name_sql"] = re.sub(r"^id_", "", args.survey)
...
def parse_args():
    ...
    parser.add_argument(
        "--source", choices=["redcap", "qualtrics"], help="A supported data source."
    )
    file_input_output = parser.add_argument_group(
        "file_input_output", "Setting File input/output path"
    )
    ...
    file_input_output.add_argument(
        ...
        help=(
            "Valid path to instrument details json file. "
            "See EXAMPLE_instrument_details.json"
        ),
    )
    qualtrics_parameters = parser.add_argument_group(
        "file_input_output", "Setting File input/output path"
    )
    parser.add_argument(
        "--apitoken", type=str, default=None, help="String of API access token."
    )
    parser.add_argument(
        "--datacenter",
        type=str,
