    ...
    try: 
        if isinstance(instrument_information["instrument_name_sql"], str):
            ...
            if (len(instrument_information["instrument_name_sql"]) >= 64): 
                raise ValueError(f'String value for `instrument_name_sql` must be 64 characters or less.\nInput value is = {len(instrument_information["instrument_name_sql"])}, which is {len(instrument_information["instrument_name_sql"])} charaters long.\nPlease update {instrument_json_path} with a valid SQL table name.')
            if (bool(re.search(r"\s", instrument_information["instrument_name_sql"]))): 
                raise ValueError(f'String value for `instrument_name_sql` cannot contain spaces.\nInput value is = {len(instrument_information["instrument_name_sql"])}.\nPlease update {instrument_json_path} with a valid SQL table name.')
    except KeyError as err: 
        raise KeyError(f"[from create_instrument_php()] the file {instrument_json_path} does not contain the key `instrument_name_sql` which is required. Update json file. ")
